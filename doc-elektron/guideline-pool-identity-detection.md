...
1. Should a per-block index/search be added for self-reported pool identity (e.g. "find all blocks self-tagged as X")? Deferred - not needed for basic display. **Resolved (September 5, 2026):** yes, for a ranking use case specifically - see [`guideline-pool-identity-ranking.md`](./guideline-pool-identity-ranking.md), implemented on the `poolrang` branch. Self-reported pools become ordinary rows in the existing `pools` table (scoped by a negative `unique_id` so they can never resolve to a real registered pool), reusing the existing ranking/hashrate/luck infrastructure rather than adding anything new - this does not revisit the "storage: none" decision in Section 5 above, which still holds for the single-block detection path this document covers.
