...
- **Version:** 0.3 (merged to `main`; fixed during live testing on `node3`, see Section 4 item 1)
...
- **`backend/src/api/pool-registry.routes.ts`** (new): `POST /api/v1/pool-registry/report`, body `{ name, blockHash }`. Looks up `name` in the synced registry map to get its **registered** URL (never the URL the caller supplied), calls back to `<url>/api/pool/identity/confirm?blockHash=<hash>` with a 5-second timeout, and only on `{ confirmed: true }` resolves/creates the pool via `SelfReportedPoolsRepository.$getOrCreatePool()` and attributes the block via the new `BlocksRepository.$updateBlockPool()`. The `/api` prefix matters: the registered URL is the pool's dashboard domain, and both reference pool repos serve their NestJS routes under a global `api` prefix behind a reverse proxy that only forwards `/api/*` to the backend - the first live test against a real pool 404'd on every callback until this was added.
- **`BlocksRepository.$updateBlockPool(hash, poolId)`** (new): `UPDATE blocks SET pool_id = ? WHERE hash = ?`, used only by the report handler above once a claim is verified. Returns whether a row was actually matched - a pool's report is a near-instant HTTP round-trip, but this server still has to fetch and process that same block itself before a row exists to attribute it to, and the report can win that race. On no match, the route handler retries a few times in the background (after already answering the pool, so its own short report timeout is never at risk) rather than dropping the report outright.
- **Live block-pool update**: once an attribution lands (immediately or via that retry), `Blocks.updateBlockPoolInMemory()` patches the in-memory recent-blocks cache in place and `WebsocketHandler.handleBlockPoolUpdate()` re-broadcasts that window as an ordinary `{ blocks: [...] }` message - the same shape the frontend already applies via `WebsocketService.handleResponse` -> `resetBlocks()`. No frontend change was needed: a viewer already looking at that exact block sees the pool appear without reloading. Deliberately not a full `handleNewBlock()` re-run, since nothing else about the block changed.
...
1. Endpoints: `POST /api/v1/pool-registry/report` (this repo) and `GET /api/pool/identity/confirm?blockHash=<hex>` (pool side, relative to the registered URL), matching payload shapes on both ends, identical between `elektron-net-pool` and `elektron-net-ppool`.
...
3. Retry/timeout: the confirmation callback has a 5-second timeout; on failure or a non-`true` answer, the report is simply not attributed, no retry. Attribution itself (once confirmed) does retry, briefly and in the background, only for the block-not-indexed-yet race described above - up to 5 attempts, 3 seconds apart, then it is dropped for good. Still not a persistent queue: a report that never gets attributed (confirmation failure, or the block still missing after that window) leaves nothing behind to reprocess later.
